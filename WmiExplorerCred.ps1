...
Function ConnectToComputer {
    ...
    &{
		...
		&{
			$cred = $host.UI.PromptForCredential('Connecting',  '', ([Environment]::UserDomainName+'\'+[Environment]::UserName), "\\$computer")
			if ($cred -ne $null)
			{
				$NetCred = $cred.GetNetworkCredential()
				$Script:ConnectionOptions.Authority = "NTLMDOMAIN:"+$Netcred.Domain
				$Script:ConnectionOptions.UserName = $Netcred.UserName
				$Script:ConnectionOptions.Password = $Netcred.Password
				$Script:ConnectionOptions.Impersonation = 'Impersonate'
				$Script:ConnectionOptions.Authentication = 'PacketPrivacy'
				#$Script:ConnectionOptions.EnablePrivileges = $true
			}
		}
