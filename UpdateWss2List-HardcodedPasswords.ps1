$ListsURL, $usr, $pwd, $udom = "https://liveserver.team.partners.extranet.microsoft.com/Companies/_vti_bin/Lists.asmx", "lcsweb", 'mYstArdustm3lody', 'redmond'
...
function SoapRequest([string] $url, [string] $namespace, [string] $method, [string] $bodyXml, [string] $usr='', [string] $pwd='', [string] $udom='')
{
...
$ListsColls = SoapRequest $ListsURL $sharepointNamespace 'GetListCollection' '' $usr $pwd $udom
...
$ListSchemaResult = SoapRequest $ListsURL $sharepointNamespace 'GetList' '<listName>Erich&apos;s Test List</listName>' $usr $pwd $udom
...
if ($null -ne $ListSchema) {
	...
	$newSchemaResult = SoapRequest $ListsURL $sharepointNamespace 'UpdateList' $modify $usr $pwd $udom
...
$WebCollResult = SoapRequest $WebsURL $sharepointNamespace 'GetWebCollection' '' $usr $pwd $udom
